                       GAMMA CygWin + MinGW Hybrid Binaries
...
the CygWin package installed (including GCC and MinGW option). Please note that
...
CygWin supplied bash shell. Place the tarball into the root directory (/) then
...
the /cygdrive/c/gamma/gamma-4.1.0/src (C:\gamma\gamma-4.1.0\src in Windows). It
will also place the DLL imports library, libgamma.dll.a, into the directory
/usr/lib. If you use a Windows GUI unzipping program such as WinZip it will not
see the "Unix-like" directory structure that CygWin uses.

This tarball originated from the GAMMA WWW site at:

        http://gamma.magnet.fsu.edu/download/binary/cygwin/index.html
