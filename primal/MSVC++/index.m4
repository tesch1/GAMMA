divert(-1)
define(_TITLE, `GAMMA Builds Using MSVC++')
define(_PRIMALDIR, `..')
...
define(_TOPNAME, MSVC++<BR>Build & Targets)
define(_CREATION,    `September 23 2002')
define(_CGIDOCNAME,  `msvc')
define(_KEYWORDS,    ``
  <META name="keywords" content="NMR, ESR, EMR, MRI, FID, resonance, science, education, simulation">'')
define(_DESCRIPTION, `
  <META name="description" content="GAMMA Builds Using Microsoft Visual C++.">')
define(_JAVASCRIPT, `')
define(_SUBDIR, msvc)
      ...
      <TABLE border=0 valign="top">
        <TR>
          <TD valign=top>
            ...
            the Microsoft Visual C++ compiler. The GAMMA libraries produced should be portable
            to any Windows based system as should any GAMMA based progam executables. 
            </FONT></STRONG></BLOCKQUOTE>

            This directory contains a hand-built Makefile for the build of Windows based
            GAMMA libraries (static and DLL) and the Windows based PyGAMMA DLL using MSVC++.
            You MUST have the Visual C++ on your computer PC system before these files will be
            of any direct use to you.  Note that for the build of PyGAMMA you must hav both
            ...
            have been built with MSVC++.  More information is available in local GAMMA documentation
            if needed. See
            <A HREF = "_DOCDIR/pygamma/python/msvc/index.html">Setting Up Python</A> and
            <A HREF = "_DOCDIR/pygamma/boost/msvc/index.html">Setting Up Boost.Python</A>.
            ...
            <P>
            Why does this Makefile exist? Typically with MSVC++ one would build GAMMA and PyGAMMA
            within the IDE using a supplied workspace. However there are other targets in the GAMMA
            Makefile that are not (AFAIK) supported in MSVC++`,' in particular those that distribute
            source codes and MSVC++ binaries. So`,' you do not need this Makefile to build either
            GAMMA or PyGAMMA if you have the MSVC++ workspaces to do so... the latter means is by
            far more preferable. This Makefile exists mostly to support other GAMMA Makefile targets.

            <P>
            This makefile is a vastly simplified version of the main GAMMA makefile.  At the time this
            page was written`,' the current version of MSVC++ is 6 (.NET is out). Builds of GAMMA were
            successfully accomplished using it.
