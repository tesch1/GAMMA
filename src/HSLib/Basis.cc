...
std::string basis::PyPrint()
  {
  std::stringstream sstr;
  std::string hdr;
  if(!dim())
    {
    hdr = std::string("Empty Basis");
    sstr << CenterString(hdr) << std::endl;
    return sstr.str();
    }
  if(isDefaultBasis())
    {
    hdr = std::string("Default Basis (")
        + Gdec(dim()) + std::string(" x ") + Gdec(dim())
        + std::string(") Identity Matrix");
    sstr << CenterString(hdr) << std::endl;
    return sstr.str();
    }
  if(bname.length()) hdr = bname;
  else               hdr = "Basis";
  sstr << (matrix)*this;
  int N = sub_N();
  if(N>1) 
...
#include <boost/python/def.hpp>				// Boost.Python def function
...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(basis_TestBasis, basis::TestBasis, 0, 1)

void PyBasis()
  {
//                                  The Class

  boost::python::class_<basis>("basis", init<>())

//                                Constructors

    .def(init<int>())
//    .def(init<const std::vector<int>>())
//    .def(init<const matrix&, int nc, int*>())
    .def(init<const basis&, const matrix&>())
...
//                                  Member Functions
    ...
    .def("dim",  &basis::dim)
    .def("size", &basis::size)
    .def("name", (std::string (basis::*)() const)             &basis::name)
    .def("name", (void        (basis::*)(const std::string&)) &basis::name)

    .def("dim_LS",         &basis::dim_LS)
    .def("sub_N",          &basis::sub_N)
    .def("sub_dim",        &basis::sub_dim)
    .def("sub_anchor",     &basis::sub_anchor)
    .def("sub_anchor_LS",  &basis::sub_anchor_LS)
    .def("which_sub_LS",   &basis::which_sub_LS)

    .def("U",              &basis::U)
    .def("get_matrix",     &basis::get_matrix)
    .def("get_mx",         &basis::get_mx)
    .def("convert",        &basis::convert)
    .def("convert_back",   &basis::convert_back)

    .def("isDefaultBasis", &basis::isDefaultBasis)
    .def("refs",           &basis::refs)
    .def("check",          &basis::check)

    .def("TestBasis",      (double(basis::*) (int))0, basis_TestBasis())
...
//                             Python Specialty Functions
    ...
    .def("__str__", &basis::PyPrint)
    ;

//                                  Global Functions

  def("defLSbasis",     defLSbasis);
  def("defbasis",       defbasis);
  def("tensor_product", (basis (*)(const basis&, const basis&)) &tensor_product);
