...
/* These functions are only for the PyGAMMA compilation. Since the Python
   ...
   such, there is no coupling of output statmenets (e.g. cout << sys1 << sys2)
   so there is no reason to worry about such issues in this instance.
  
           Input                sys     : A basic spin system  (this) 
                                ostr    : Output stream 
           Output               none    : System info placed into the
                                          output stream ostr
...
void           spin_sys::isotopeSSA(int i, const std::string& s) { isotope(i,s); }
void           spin_sys::isotopeSSB(int i, const Isotope& I)     { isotope(i,I); }
const Isotope& spin_sys::isotopeSSC(int i) const                 { return isotope(i); }

double         spin_sys::gammaSSA(int spin)               const  { return gammaSSA(spin); }
double         spin_sys::gammaSSB(const std::string& iso) const  { return gammaSSB(iso);  }

int         spin_sys::isotopesSSA()                     const { return isotopes(); }
std::string spin_sys::isotopesSSB(int idx)              const { return isotopes(idx); }
bool        spin_sys::isotopesSSC(const std::string& I) const { return isotopes(I); }

void spin_sys::SetFlagsSSA(bool TF)                           { SetFlags(TF); }
void spin_sys::SetFlagsSSB(const std::string& isoin, bool TF) { SetFlags(isoin, TF); }
void spin_sys::SetFlagsSSC(const Isotope& Iso, bool TF)       { SetFlags(Iso, TF); }

      void         spin_sys::nameSSA(const std::string& n)    { name(n); }
const std::string& spin_sys::nameSSB() const                  { return name(); }
const std::string& spin_sys::nameSSC(int i) const             { return name(i); }

void spin_sys::warningsSSA(int warnf) { warnings(warnf); }
int  spin_sys::warningsSSB() const    { return warnings(); }

std::string  spin_sys::IsoDefaultSSA()                       { return IsoDefault(); }
void         spin_sys::IsoDefaultSSB(const std::string& DI)  { IsoDefault(DI); }
...
flagvec spin_sys::GetFlagsSYS1() const { return spinflags; }

flagvec spin_sys::GetFlagsSYS2(bool TF) const
  { return GetFlags(TF); }

flagvec spin_sys::GetFlagsSYS3(int spin, bool TF, bool DefTF) const
  { return GetFlags(spin, TF, DefTF); }

flagvec spin_sys::GetFlagsSYS4(const std::string& isoin, bool TF, bool DefTF) const
  { return GetFlags(isoin, TF, DefTF); }

flagvec spin_sys::GetFlagsSYS5(const Isotope& isoin, bool TF, bool DefTF) const
  { return GetFlags(isoin, TF, DefTF); }
...
/* The read functions have default parameter values which force the users into
   inputting more arguments in the function call. Here we simply add a couple
...
int spin_sys::readSYS1(const std::string& filename) 
  { return read(filename,-1,2); }

int spin_sys::readSYS2(const std::string& filename, int idx)
  { return read(filename,idx,2); }
...
std::string spin_sys::PyPrint()
  ...
  if(rs>0) lst += std::string(rs, ' ');		// Adjust for centering
  for(int i=0; i<nr; i++)			// Print rows. Rows each have
    sstr << lst << Strs[i];			// different data, cols are
  return sstr.str();
  }
...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(spinsys_HS,       spin_sys::HS, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(spinsys_qn,       spin_sys::qn, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(spinsys_momentum, spin_sys::momentum, 0, 1)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(spinsys_read,     spin_sys::read,  1, 3)

void PySpinSys()
  {
//                                  The Class

  boost::python::class_<spin_sys>("spin_sys", init<>())
...
//                              Member Functions
    ...
    .def("spins",         &spin_sys::spins)
    .def("spinpairs",     &spin_sys::spinpairs)

    .def("HS",            (int(spin_sys::*) (int))0, spinsys_HS())
    .def("isotope",       &spin_sys::isotopeSSA)
    .def("isotope",       &spin_sys::isotopeSSC,  return_value_policy<copy_const_reference>())
    .def("weight",        &spin_sys::weight)
    .def("symbol",        &spin_sys::symbol)
    .def("qn",            (double(spin_sys::*) (int))0, spinsys_qn())
    .def("element",       &spin_sys::element)
    .def("momentum",      (std::string(spin_sys::*) (int))0, spinsys_momentum())
    .def("gamma",         &spin_sys::gammaSSA)
    .def("gamma",         &spin_sys::gammaSSB)
//    .def("IsoVec",        &spin_sys::IsoVec)

//    .def("HSvect",        &spin_sys::HSvect)
    .def("qState",        &spin_sys::qState)
    .def("qnState",       &spin_sys::qnState)
    .def("qnStates",      &spin_sys::qnStates)
    .def("qnDist",        &spin_sys::qnDist)
    .def("CoherDist",     &spin_sys::CoherDist)

    .def("homonuclear",   &spin_sys::homonuclear)
    .def("heteronuclear", &spin_sys::heteronuclear)
    .def("electron",      &spin_sys::electron)
    .def("nucleon",       &spin_sys::nucleon)
    .def("spinhalf",      &spin_sys::spinhalf)
    .def("electrons",     &spin_sys::electrons)
    .def("nucleons",      &spin_sys::nucleons)
    .def("nepair",        &spin_sys::nepair)
    .def("enpair",        &spin_sys::enpair)
    .def("eepair",        &spin_sys::eepair)
    .def("nnpair",        &spin_sys::nnpair)
    .def("pairidx",       &spin_sys::pairidx)
    .def("isotopes",      &spin_sys::isotopesSSA)
    .def("isotopes",      &spin_sys::isotopesSSB)
    .def("isotopes",      &spin_sys::isotopesSSC)

    .def("SetFlag",       &spin_sys::SetFlag)
    .def("SetFlags",      &spin_sys::SetFlagsSSA)
    .def("SetFlags",      &spin_sys::SetFlagsSSB)
    .def("SetFlags",      &spin_sys::SetFlagsSSC)
    .def("GetFlag",       &spin_sys::GetFlag)

// Missing GetFlags functions

    .def("name",        &spin_sys::nameSSA)
    .def("name",        &spin_sys::nameSSB,  return_value_policy<copy_const_reference>())
    .def("name",        &spin_sys::nameSSC,  return_value_policy<copy_const_reference>())

    .def("warnings",    &spin_sys::warningsSSA)
    .def("warnings",    &spin_sys::warningsSSB)

    .def("IsoDefault",  &spin_sys::IsoDefaultSSA)
    .def("IsoDefault",  &spin_sys::IsoDefaultSSB)

// Missing Parameter Set functions

// Missing write functions

    .def("read", (int(spin_sys::*) (const std::string&,  int, int))0, spinsys_read())
    .def("read", (int(spin_sys::*) (ParameterSet&,       int, int))0, spinsys_read())

// Missing ask read functions

    .def("get_basis",      &spin_sys::get_basis)      
    .def("BasisMap1",      &spin_sys::BasisMap1)
    .def("TransitionMap1", &spin_sys::TransitionMap1)
    .def("BasisMap2",      &spin_sys::BasisMap2)
    .def("TransitionMap2", &spin_sys::TransitionMap2)

//                      Python Specialty Functions

    .def("__str__", &spin_sys::PyPrint)
  ;
  }
