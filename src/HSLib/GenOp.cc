...
std::string gen_op::PyPrint()
  ...
  if(!WBR)
    {
    hdr = std::string("Empty Hilbert Space Operator");
    sstr << CenterString(hdr) << std::endl;
    return sstr.str();
    }
  hdr = std::string("Hilbert Space Operator");
...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_trace, gen_op::trace, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_exp,   gen_op::exp,   0, 2)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_Op_base,         gen_op::Op_base, 1, 2)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_TestEigenSystem, gen_op::TestEigenSystem, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_status,          gen_op::status, 0, 1)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_is_symmetric, gen_op::is_symmetric, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_is_hermitian, gen_op::is_hermitian, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_is_unitary,   gen_op::is_unitary,   0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_is_real,      gen_op::is_real,      0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_is_imaginary, gen_op::is_imaginary, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_is_complex,   gen_op::is_complex,   0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_is_zero,      gen_op::is_zero,      0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(genop_is_diagonal,  gen_op::is_diagonal,  0, 1)

void PyGenOp()
  {
//                                  The Class

  boost::python::class_<gen_op>("gen_op", init<>())

//                                Constructors

    .def(init<const matrix&>())
    .def(init<const spin_op&>())
    .def(init<const matrix&, const matrix&>())
    .def(init<const matrix&, const basis&>())
    .def(init<const gen_op&>())
//    .def(init<const std::vector<matrix>&, const std::vector<matrix>&>())
//    .def(init<matrix*, int, matrix*>())
//    .def(init<int, matrix*, matrix*>())
...
//                                  Member Functions
    ...
    .def("det",   &gen_op::det)
    .def("trace", (complex(gen_op::*) (const gen_op&))0, genop_trace())
    .def("proj",  &gen_op::proj)

    .def("dim",    &gen_op::dim)
    .def("HS",     &gen_op::HS)
    .def("LS",     &gen_op::LS)
    .def("dim_LS", &gen_op::dim_LS)

    .def("exp",          (gen_op(gen_op::*) (const complex&, double))0, genop_exp())
    .def("Pow",          &gen_op::Pow)
    .def("sim_trans",    &gen_op::sim_trans)
    .def("sim_trans_ip", &gen_op::sim_trans_ip)
/*
still missing some
    .def((gen_op (gen_op::*)()                       const) &gen_op::exp, "exp");
    .def((gen_op (gen_op::*)(const complex&, double) const) &gen_op::exp, "exp");
   .def((row_vector (gen_op::*)()       const) &gen_op::eigvals, "eigvals");
// .def((void (gen_op::*)(double*, int) const) &gen_op::eigvals, "eigvals");
*/

    .def("get_mx",     &gen_op::get_mx)
    .def("get_matrix", &gen_op::get_matrix)
    .def("put_mx",     &gen_op::put_mx)
    .def("put_matrix", &gen_op::put_matrix)

    .def("get_bs",    &gen_op::get_bs)
    .def("get_basis", &gen_op::get_basis)
    .def("put_bs",    &gen_op::put_bs)
    .def("put_basis", &gen_op::put_basis)

    .def("get", &gen_op::get)
    .def("put", &gen_op::put)

    .def("bsname", &gen_op::bsname)
    .def("exists", &gen_op::exists)

// A few functions missing here


    .def("test_DBR", &gen_op::test_DBR)
    .def("test_EBR", &gen_op::test_EBR)
    .def("in_DBR",   &gen_op::in_DBR)
    .def("in_EBR",   &gen_op::in_EBR)

    .def("set_DBR",  &gen_op::set_DBR)
    .def("set_EBR",  &gen_op::set_EBR)
    .def("Op_base", (void(gen_op::*) (const gen_op &, double))0, genop_Op_base())
    .def("Op_base", (void (gen_op::*)(const basis&)    const) &gen_op::Op_base)

    .def("status",    (void(gen_op::*) (int))0, genop_status())
    .def("setOnlyWBR", &gen_op::setOnlyWBR)

    .def("Op_priority", &gen_op::Op_priority)
    .def("SetLimits",   &gen_op::SetLimits)

    .def("TestEigenSystem", (double(gen_op::*) (int))0, genop_TestEigenSystem())

/*
    .def("is_symmetric", (bool(gen_op::*) (const double))0, genop_is_symmetric())
    .def("is_hermitian", (bool(gen_op::*) (const double))0, genop_is_hermitian())
    .def("is_unitary",   (bool(gen_op::*) (const double))0, genop_is_unitary())
    .def("is_real",      (bool(gen_op::*) (const double))0, genop_is_real())
    .def("is_imaginary", (bool(gen_op::*) (const double))0, genop_is_imaginary())
    .def("is_complex",   (bool(gen_op::*) (const double))0, genop_is_complex())
    .def("is_zero",      (bool(gen_op::*) (const double))0, genop_is_zero())
*/
//    .def("is_diagonal",  (bool(gen_op::*) (const double))0, genop_is_diagonal())
    .def("is_square",     &gen_op::is_square)
...
//                      Python Specialty Functions
    ...
    .def("__str__", &gen_op::PyPrint)

//                              Global Functions

/*
    PyModule.def((gen_op (*)(const gen_op&, const gen_op&)) &tensor_product, "tensor_product");

    PyModule.def((gen_op (*)(const gen_op&)) &log,            "log"); 
    PyModule.def((gen_op (*)(const gen_op&)) &adjoint,        "adjoint");
*/
  ;
  }
