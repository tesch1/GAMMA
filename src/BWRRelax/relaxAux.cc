...
** 	              G A M M A				**
**							**
**	NMR Library, Relaxation Auxiliary Functions 	**
**						 	**
**	Implementation   				**
**						 	**
...
** 	Description				 	**
**						 	**
**	The following functions are helpful in dealing	**
**	with relaxation phenomena			**
**						 	**
*********************************************************/

#ifndef _relax_aux_cc_		// Is this file already included?
#define _relax_aux_cc_ 1	// If no, then remember it
#  if defined(GAMPRAGMA)			// Using the GNU compiler?
#    pragma implementation          // This is the implementation
#endif


#include <BWRRelax/relaxAux.h>		// Include the interface
...
/* This is now in LSLib/LSAux.cc 
void print(super_op& LOp, double cutoff, int nc, int ri)
  ...
  if(ls >= 1000)
    iacc =4;
  string Siacc = string("%d") + Gdec(iacc);
  complex Relc;
  double Reld;
  int k=0, np=0;
    ...
    for(int i=0; i< ls; i++)
      for(int j=0; j< ls; j++)
        ...
        if(ri == 1)				// Deal with complex elements
          ...
          if(norm(Relc) > cutoff) 
            {
            cout << "<" << Gdec(Siacc,i) << "|" << Gdec(Siacc,j)
                 << "> = " << Re(Relc)
                 << ", " << Im(Relc) << "\t";
            k++;
            np++;
...
/* These are now in LSLib/LSAux.cc 
...
//		Now Solve U|x> = |y> For |x>
  ...
  for(i=n-1; i>=0; i--)			// Works backwards, |x(n-1)>
    ...
    for(int j=i+1; j<n; j++)
      sum -= ALU.get(i,j)*b.get(j,0);
    b(i,0) = sum/ALU.get(i,i);		// This is now |x(i)>
    }
  return;
  }
*/
...
//return Ainv(A.rows(), A.rows(), 0.0);
  ...
  int n = A.rows();			// Get dimension of A
  int *indx;				// Set up indexing vector
  indx = new int[n];
