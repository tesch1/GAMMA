...
/* Since this code now supports PyGAMMA (GAMMA exported to run interactively
   in Python), we must use different constructors for our matrices. This is
   due to the fact that Boost (used to export C++ into Python) does not handle
   overloaded constructors when the arguments have default settings. Thus,
   we just make many more constructors to handle all possibilities. Older
   constructors are commented out.
...
/* There are two problems with the GAMMA matrix constructors with respect to
   ...
   enumeration internally.
               ...
               Arguments                           Construction Type
   ==================================    ======================================
            int, int, string                   matrix(rows, columns, type)
...
matrix(int i, int j,                   std::string type);
matrix(int i, int j,                   std::string type, std::string htype);
matrix(int i, int j,       double d,   std::string type);
matrix(int i, int j,       double d,   std::string type, std::string htype);
matrix(int i, int j, const complex& z, std::string type);
matrix(int i, int j, const complex& z, std::string type, std::string htype);
...
friend matrix mxadd(matrix& self, const matrix& right);
friend matrix mxsub(matrix& self, const matrix& right);
friend matrix mxmul(matrix& self, const matrix& right);
friend matrix mxdiv(matrix& self, const matrix& right);

friend matrix mxmulz(matrix& self, const complex& right);
friend matrix mxmuld(matrix& self,       double   right);
friend matrix mxdivz(matrix& self, const complex& right);
friend matrix mxdivd(matrix& self,       double   right);

// This operator allow use of - (negation) for matrices in Python

friend matrix mxneg(const matrix& self);
