...
#include <boost/python/bases.hpp>			// Boost.Python base classes
...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(colvec_max,  col_vector::max,  0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(colvec_min,  col_vector::min,  0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(colvec_sort, col_vector::sort, 0, 1)
...
std::string col_vector::PyPrint() const
  ...
  MxPrint PFlgs = matrix::PrintFlags();	// Use matrix print flags
  std::stringstream sstr;		// Like ostream but with strings
  int len;				// For centering
  if(PFlgs.MxHdr)			// If we desire a header output
    {
    std::string hdr = hdrString();
    len = 40-hdr.length()/2;
  ...
  if(!ns) return sstr.str();
  len = 40-((PStrings[0]).length())/2;	// Spaces for centering data
  std::string csp("");			// If no spaces
  if(len>0) csp = std::string(len, ' ');// If some spaces
  for(unsigned i=0; i<ns; i++)		// Loop over strings and send
    sstr << csp << PStrings[i]		// each line to our string stream
         << std::endl;
...
void   col_vector::putCVEC(double d, int i) { put(complex(d,0),i); }

//-----------------------------------------------------------------------------
//                            Simple Unary Functions
//-----------------------------------------------------------------------------

complex col_vector::sum1()               const { return sum(); }
complex col_vector::sum2(int st, int ne) const { return sum(st,ne); }
...
col_vector col_vector::Pyproduct2(const row_vector& rvec) const
  { return product(rvec); }
col_vector col_vector::Pyproduct3(const col_vector& cvec) const
  { return product(cvec); }

double  col_vector::Pyscalar_product1()                        const
  { return scalar_product(); }
complex col_vector::Pyscalar_product2(const col_vector& cvec)  const
  { return scalar_product(cvec); }
complex col_vector::Pyscalar_product3(const row_vector& rvec)  const
  { return scalar_product(rvec); }
...
//                                The Class
  ...
  class_<col_vector, bases<matrix> >("col_vector")
...
//                              Constructors
    ...
    .def(init<int>                    ())
    .def(init<int, double>            ())
    .def(init<int, const complex&>    ())
//    .def(init<int, const col_vector&> ())
//    .def(init<int, const matrix&>     ())

    .def("get",      &col_vector::get)
    .def("getRe",    &col_vector::getRe)
    .def("getIm",    &col_vector::getIm)
    .def("put",      &col_vector::put)

    .def("elements", &col_vector::elements)
    .def("size",     &col_vector::size)

    .def("norm",     &col_vector::norm)
    .def("maxRe",    &col_vector::maxRe)
    .def("maxIm",    &col_vector::maxIm)
    .def("maxZ",     &col_vector::maxZ)
    .def("minRe",    &col_vector::minRe)
    .def("minIm",    &col_vector::minIm)
    .def("minZ",     &col_vector::minZ)
    .def("flip",     &col_vector::flip)
    .def("zero",     &col_vector::zero)

/* Note that the function sort returns a STL vector of integers. As such it
   must have a class of vector<int> exported into Python. That can be found
   in the file PyBoost.{h,cc} in the GAMMA source directory, src. The  function
   sum has two distinct overloads which are quite different in their argument
   lists. These have to be uniquely defined and exported individually.         */

    .def("max",  (int(col_vector::*) (int) const)0, colvec_max())
    .def("min",  (int(col_vector::*) (int) const)0, colvec_min())
    .def("sort", (std::vector<int>(col_vector::*) (int) const)0, colvec_sort())
    .def("sum",  &col_vector::sum1)
    .def("sum",  &col_vector::sum2)

//                          Product Functions

    .def("product",            &col_vector::Pyproduct1)
    .def("product",            &col_vector::Pyproduct2)
    .def("product",            &col_vector::Pyproduct3)

    .def("scalar_product",     &col_vector::Pyscalar_product1)
    .def("scalar_product",     &col_vector::Pyscalar_product2)
    .def("scalar_product",     &col_vector::Pyscalar_product3)

//                          Python Specialty Functions

    .def("__str__",     &col_vector::PyPrint)
    ;
  }
