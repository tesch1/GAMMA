...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(rowvec_max,  row_vector::max,  0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(rowvec_min,  row_vector::min,  0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(rowvec_sort, row_vector::sort, 0, 1)
...
void row_vector::putRVEC(double d, int i) { put(complex(d,0),i); }

col_vector adjoint_ROW_VECTOR_py1(const row_vector& rvec)
  { return col_vector(adjoint((const matrix &)rvec)); }

col_vector transpose_ROW_VECTOR_py1(const row_vector& rvec)
  { return col_vector(transpose((const matrix &)rvec)); }
 
complex trace_ROW_VECTOR_py1(const row_vector& rvec)
  { return rvec.trace(); }

row_vector FFT_ROW_VECTOR_py1(const row_vector& rvec)  { return FFT((const matrix &)rvec);  }
row_vector IFFT_ROW_VECTOR_py1(const row_vector& rvec) { return IFFT((const matrix &)rvec); }

double scalar_product_ROW_VECTOR_py1(const row_vector& rvec)
  {
  int dim = rvec.size();
  double sp=0;
  for(int i=dim-1; i>=0; i--)
    sp += square_norm(rvec.get(i));
  return sp;
  }

complex scalar_product_ROW_VECTOR_py2(const row_vector& rvec1, const row_vector& rvec2)
  {
  int dim = rvec1.size();
  if(dim != rvec2.size())
    {
    rvec1.RVerror(42, 1);                             // Vec-Vec dimensions
    rvec1.RVerror(21,"Row Vector (x) Row Vec");       // Problems in this
    std::string pname("scalar_product");
    rvec1.RVfatality(5, pname);
    }
  complex z(0);
  for(int i=dim-1; i>=0; i--)
    z += conj(rvec1.get(i),rvec2.get(i));
  return z;
  }

complex scalar_product_ROW_VECTOR_py3(const row_vector& rvec, const col_vector& cvec)
  {
  int dim = rvec.size();
  if(dim != cvec.size())
    {
    rvec.RVerror(42, 1);                             // Vec-Vec dimensions
    rvec.RVerror(21,"Row Vector (x) Column Vec");       // Problems in this
    std::string pname("scalar_product");
    rvec.RVfatality(5, pname);
    }
  complex z(0);
  for(int i=dim-1; i>=0; i--)
    z += rvec.get(i)*cvec.get(i);
  return z;
  }
...
//                                The Class
  ...
  class_<row_vector, bases<matrix> >("row_vector")
...
//    .def(init<int, const row_vector&> ())
//    .def(init<int, const matrix&>     ())

    .def("get",      &row_vector::get)
    .def("getRe",    &row_vector::getRe)
    .def("getIm",    &row_vector::getIm)
    .def("put",      &row_vector::put)

    .def("elements", &row_vector::elements)
    .def("size",     &row_vector::size)

    .def("norm",     &row_vector::norm)
    .def("maxRe",    &row_vector::maxRe)
    .def("maxIm",    &row_vector::maxIm)
    .def("maxZ",     &row_vector::maxZ)
    .def("minRe",    &row_vector::minRe)
    .def("minIm",    &row_vector::minIm)
    .def("minZ",     &row_vector::minZ)
    .def("flip",     &row_vector::flip)
    .def("zero",     &row_vector::zero)

    .def("max",  (int(row_vector::*) (int) const)0, rowvec_max())
    .def("min",  (int(row_vector::*) (int) const)0, rowvec_min())

//                          Python Specialty Functions

    .def("__str__",     &row_vector::PyPrint)
    ;
