...
#include <Matrix/py_matrix.h>		// Include PyGAMMA matrix (MSVC++ V6)
#include <Matrix/pyy_matrix.h>		// Include PyGAMMA matrix (MSVC++ V6)
...
// ____________________________________________________________________________
// R                          PyGAMMA Code (Member)
// ____________________________________________________________________________

/* There are two problems with the GAMMA matrix constructors with respect to
   exporting then into Python using Boost.Python. First Boost.Python does not
   handle argument default values. To compensate, more constructors are added
   to take care of all possible constructor variations. Second, Boost.Python
   does not handle enumerations well. This is used to specify particular matrix
   types (e.g. d_matrix_type). To compensate, we add constructors that take a
   string to specify the matrix type, then translate this string into an
   enumeration internally.                                                    */
...
/* There are not even Pythonized, they just use strings rather than enum to set
   the matrix and hermitian types. This is so Python does not have to rely
   on anything using enumerations.                                           */

matrix::matrix(int i, int j, std::string type)
  {
       if (type == "d_matrix_type") m = virtual_copy(new d_matrix(i,j,0));
  else if (type == "i_matrix_type") m = virtual_copy(new i_matrix(i,j));
  else if (type == "h_matrix_type") m = virtual_copy(new h_matrix(i,j,0));
  else                              m = virtual_copy(new n_matrix(i,j,0));
  }

matrix::matrix(int i, int j, double d, std::string type)
  {
       if (type == "d_matrix_type") m = virtual_copy(new d_matrix(i,j,d));
  else if (type == "i_matrix_type") m = virtual_copy(new i_matrix(i,j,d));
  else if (type == "h_matrix_type") m = virtual_copy(new h_matrix(i,j,d));
  else                              m = virtual_copy(new n_matrix(i,j,d));
  }

matrix::matrix(int i, int j, const complex& z, std::string type)
  {
       if (type == "d_matrix_type") m = virtual_copy(new d_matrix(i,j,z));
  else if (type == "i_matrix_type") m = virtual_copy(new i_matrix(i,j,z));
  else if (type == "h_matrix_type") m = virtual_copy(new h_matrix(i,j,z));
  else                              m = virtual_copy(new n_matrix(i,j,z));
  }

matrix::matrix(int i, int j, std::string type, std::string htype)
  {
  if (type == "d_matrix_type") m = virtual_copy(new d_matrix(i,j,0));
	else if (type == "i_matrix_type")
		m = virtual_copy(new i_matrix(i,j,0));
	else if (type == "h_matrix_type")
		m = virtual_copy(new h_matrix(i,j,0));
	else m = virtual_copy(new n_matrix(i,j,0));
  }

matrix::matrix(int i, int j, double d, std::string type, std::string htype)
  {
       if (type == "d_matrix_type") m = virtual_copy(new d_matrix(i,j,d));
  else if (type == "i_matrix_type") m = virtual_copy(new i_matrix(i,j,d));
  else if (type == "h_matrix_type") m = virtual_copy(new h_matrix(i,j,d));
...
matrix::matrix(int i, int j, const complex& z, std::string type, std::string htype)
  {
       if (type == "d_matrix_type") m = virtual_copy(new d_matrix(i,j,z));
  else if (type == "i_matrix_type") m = virtual_copy(new i_matrix(i,j,z));
  else if (type == "h_matrix_type") m = virtual_copy(new h_matrix(i,j,z));
...
//void matrix::put(double d, int i, int j) { put(complex(d), i, j); }
//void matrix::put_h(      double   d, int i, int j) { put_h(complex(d), i,j); }
void matrix::putMX1(double d, int i, int j) { put(complex(d,0),i,j); }
void matrix::putMX2(double d, int i, int j) { put_h(complex(d,0), i, j); }
...
std::string matrix::stored_hermitianMX( ) const 
  {
  hermitian_type h = m->stored_hermitian();
  if (h == _hermitian) return "hermitian";
  else return "non_hermitian";
  }

std::string matrix::test_hermitianMX(double d) const
  {
  hermitian_type h = m->test_hermitian(d);
  if (h == _hermitian) return "hermitian";
  else return "non_hermitian";
  }
...
void matrix::set_hermitianMX(std::string htype)
  {
  hermitian_type ht;
  if(htype == "_hermitian") ht = _hermitian;
  else                      ht = non_hermitian;
  set_hermitian(ht);
  }
      
std::string matrix::check_hermitianMX(double d)
  {
  std::string h = test_hermitianMX(d);	// First test if it can be Hermitian
  set_hermitianMX(h);			// Set the type accordingly
  return h;					// Return T/F depending on test
  }
...
std::string matrix::stored_typeMX() const
  {
  matrix_type x = m->stored_type();
  switch(x)
    {
    case n_matrix_type: return "n_matrix_type";
    case d_matrix_type: return "d_matrix_type";
    case i_matrix_type: return "i_matrix_type";
    case h_matrix_type: return "h_matrix_type";
    default: return "_matrix_type";
    }
  }
...
std::string matrix::test_typeMX(std::string type, double d) const
  {
  matrix_type mt;
       if (type == "d_matrix_type") mt = d_matrix_type;
  else if (type == "i_matrix_type") mt = i_matrix_type;
  else if (type == "h_matrix_type") mt = h_matrix_type;
  else                              mt = n_matrix_type;
  mt = test_type(mt);
  std::string sret;
  switch(mt)
    {
    case d_matrix_type: sret = std::string("d_matrix_type");
    case i_matrix_type: sret = std::string("i_matrix_type");
    case h_matrix_type: sret = std::string("h_matrix_type");
    default:            sret = std::string("n_matrix_type");
    }
  return sret;
  }

void matrix::set_typeMX(std::string type)
  {
  matrix_type mt;
       if (type == "d_matrix_type") mt = d_matrix_type;
  else if (type == "i_matrix_type") mt = i_matrix_type;
...
std::string matrix::check_typeMX(std::string type, double d)
  {
  std::string tmp = test_typeMX(type,d);
  set_typeMX(tmp);
  return tmp;
  }

// -------------------------------------------------------------------------
//      Added Binary Operators For Treating Column And Row Vectors
// -------------------------------------------------------------------------
// sosi
/*
col_vector matrix::operator + (const col_vector& cv) const
  { return cv + mx; }
col_vector matrix::operator - (const col_vector& cv) const
  { return -(cv - mx); }
//row_vector matrix::operator * (const col_vector& cv) const;
//  { return (cv & mx); }
*/
...
// These operators allow use of +=, -=, etc. for matrices in Python

matrix mxadd(matrix& self, const matrix& right)
  { return matrix(self) += right; }
matrix mxsub(matrix& self, const matrix& right)
  { return matrix(self) -= right; }
matrix mxmul(matrix& self, const matrix& right)
  { return matrix(self) *= right; }
matrix mxdiv(matrix& self, const matrix& right)
  { return matrix(self) /= right; }

matrix mxmulz(matrix& self, const complex& right)
  { return matrix(self) *= right; }
matrix mxmuld(matrix& self, double right)
  { return matrix(self) *= right; }
matrix mxdivz(matrix& self, const complex& right)
  { return matrix(self) /= right; }
matrix mxdivd(matrix& self, double right)
  { return matrix(self) /= right; }

// This operator allow use of - (negation) for matrices in Python

matrix mxneg(const matrix& self)
...
std::string matrix::PyPrint()
  ...
  int len;
  if(PrntFlgs.MxHdr)				// If we desire a header output
    {
    std::string hdr = MxModdec(rows()) 
                    + std::string(" x ") 
                    + MxModdec(cols());
    hdr += std::string(" ") + m->mxtype(PrntFlgs.MxRIPrnt);
    hdr += std::string(" Matrix");
    len = 40-hdr.length()/2;
    if(len>0) sstr << std::string(len, ' ');
    sstr << hdr << std::endl << std::endl;
    }
  std::vector<std::string> PStrings;		// Array of strings to print
  if(rows() > PrntFlgs.MxPctDim)		// For a very large matrix 
    PStrings = m->pictureStrings(PrntFlgs);	// we just print a picture
  else						// Otherwise we print out
    PStrings = m->printStrings(PrntFlgs);	// the array elements
  unsigned ns = PStrings.size();		// No. of strings to print
  std::string ctr("");
  if(ns)
    {
    len = 40-((PStrings[0]).length())/2;	// Amount needed center line
    if(len>0) ctr = std::string(len,' ');
    for(unsigned i=0; i<ns; i++)		// Loop over strings and send
    sstr << ctr << PStrings[i] << std::endl;	// to our string stream
    }
  return sstr.str();				// Return as Python printable
  }
...
#include <boost/python/def.hpp>				// Boost.Python function defs
#include <boost/python/overloads.hpp>			// Boost.Python overloads
#include <boost/python/enum.hpp>			// Boost.Python enumerations
...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_is_symmetric, matrix::is_symmetric, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_is_hermitian, matrix::is_hermitian, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_is_unitary,   matrix::is_unitary,   0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_is_real,      matrix::is_real,      0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_is_imaginary, matrix::is_imaginary, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_is_complex,   matrix::is_complex,   0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_is_zero,      matrix::is_zero,      0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_is_diagonal,  matrix::is_diagonal,  0, 1)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_check_hermitian,  matrix::check_hermitian, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_set_hermitian,    matrix::set_hermitian,   0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_test_hermitian,   matrix::test_hermitian,  0, 1)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_test_type,  matrix::test_type,   1, 2)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_check_type, matrix::check_type,  1, 2)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(matrix_trace, matrix::trace,  0, 1)
...
void PyMatrix()
  ...
  boost::python::enum_<matrix_type>("matrix_type")
        .value( "_matrix_type",  _matrix_type)
        .value("n_matrix_type", n_matrix_type)
        .value("d_matrix_type", d_matrix_type)
        .value("i_matrix_type", i_matrix_type)
        .value("h_matrix_type", h_matrix_type)
        ;

  boost::python::enum_<hermitian_type>("hermitian_type")
        .value( "non_hermitian",  non_hermitian)
        .value(    "_hermitian",     _hermitian)
        ;

/* Note the use of PyMx below (next two code lines). This is done only
   to appease MSVC++ V6. See other notes later on & use of the function
   PyMatrixMore(). sosi                                                  */

//                                The Class

  boost::python::class_<matrix> PyMx("matrix", init<>());

//                                Constructors
  PyMx
    .def(init<int, int> ())
    .def(init<int, int, matrix_type> ())
    .def(init<int, int, matrix_type,  hermitian_type> ())

    .def(init<int, int, double>())
    .def(init<int, int, double, matrix_type> ())
    .def(init<int, int, double, matrix_type,  hermitian_type> ())

    .def(init<int, int, const complex &>())
    .def(init<int, int, const complex &, matrix_type> ())
    .def(init<int, int, const complex &, matrix_type,  hermitian_type> ())
...
//                          Member Access Functions
    ...
    .def("get",       &matrix::get)
    .def("getRe",     &matrix::getRe)
    .def("getIm",     &matrix::getIm)
    .def("put",       &matrix::put)
    .def("put_h",     &matrix::put_h)
    .def("get_block", &matrix::get_block)
    .def("put_block", &matrix::put_block)

//                     Matrix Hermitian Type Handling

    .def("stored_hermitian",  &matrix::stored_hermitian)
    .def("check_hermitian",  (hermitian_type(matrix::*) (double)        )0, matrix_check_hermitian())
    .def("set_hermitian",    (void          (matrix::*) (hermitian_type))0, matrix_set_hermitian())
    .def("test_hermitian",   (hermitian_type(matrix::*) (double)   const)0, matrix_test_hermitian())

//                          Matrix Type Handling

    .def("stored_type", &matrix::stored_type)
    .def("set_type",    &matrix::set_type)
    .def("mxtype",      &matrix::mxtype)
    .def("test_type",  (matrix_type(matrix::*) (matrix_type, double) const)0, matrix_test_type())
    .def("check_type", (matrix_type(matrix::*) (matrix_type, double)      )0, matrix_check_type())

//                    Matrix Checks and Parameters

    .def("cols",      &matrix::cols)
    .def("rows",      &matrix::rows)
    .def("refs",      &matrix::refs)
    .def("pts",       &matrix::pts)

    .def("is_symmetric", (bool(matrix::*) (double) const)0, matrix_is_symmetric())
    .def("is_hermitian", (bool(matrix::*) (double) const)0, matrix_is_hermitian())
    .def("is_unitary",   (bool(matrix::*) (double) const)0, matrix_is_unitary())
    .def("is_real",      (bool(matrix::*) (double) const)0, matrix_is_real())
    .def("is_imaginary", (bool(matrix::*) (double) const)0, matrix_is_imaginary())
    .def("is_complex",   (bool(matrix::*) (double) const)0, matrix_is_complex())
    .def("is_zero",      (bool(matrix::*) (double) const)0, matrix_is_zero())
    .def("is_diagonal",  (bool(matrix::*) (double) const)0, matrix_is_diagonal())
    .def("is_square",    &matrix::is_square)

/* Due to limitations in MS Visual C++ Version 6.0, one cannot have too big of
   code blocks without generating the following

           fatal error C1204: compiler limit : internal structure overflow
  
   Unless this can be fixed in the Boost.Python code (or MS fixes it....right)
   then we must move the rest of BP class matrix to another file. See the file
   py_matrix(.h,.cc) for the rest of the matrix exports.                         */

//                  Matrix Operators (see py_matrix.cc)

    .def("Re",        &matrix::Re)
    .def("Im",        &matrix::Im)
    .def("conj",      &matrix::conj)
    .def("transpose", &matrix::transpose)
    .def("adjoint",   &matrix::adjoint)
    .def("exp",       &matrix::exp)
    .def("trace",    (complex(matrix::*) (const matrix&, const matrix&) const)0, matrix_trace())


/* The previous function export the member, not the non-member functions.
   Note that the function trace is not yet exported. It has some overloads
   that mke it harder to do that.... sosi                 */

    .def("swaprows", &matrix::swaprows)
    .def("swapcols", &matrix::swapcols)
    .def("permute",  &matrix::permute)
    .def("maxRe",    &matrix::maxRe)
    .def("maxIm",    &matrix::maxIm)
    .def("maxZ",     &matrix::maxZ)
    .def("minRe",    &matrix::minRe)
    .def("minIm",    &matrix::minIm)
    .def("minZ",     &matrix::minZ)

//    .def(&matrix::det,      "det");
//    .def(&matrix::rank,     "rank");

    .def("FFT",  &matrix::FFT)
    .def("IFFT", &matrix::IFFT)

/* These are likely not working at the moment since they are static. Current
   Boost.Python does not handle static functions properly (V2) so until that
   is fixed we must use some global functions instead. See below. Note that
   PrintFlags returns MxPrint which has not been defined. sosi             */

//    .def("Header",    &matrix::Header)
//    .def("PrintRI",   &matrix::PrintRI)
//    .def("PrintAll",  &matrix::PrintAll)
//    .def("PictDim",   &matrix::PictDim)
//    .def("PrintVal",  &matrix::PrintVal)
//    .def("PrintCols", &matrix::PrintCols)
//    .def("PrintRows", &matrix::PrintRows)
//    .def("PrintFlags", &matrix::PrintFlags)

// Most of the output functions are not exported either....sosi
...
//            Matrix Diagonalization Functions (See py_matrix)
...
//             Matrix Testing Functions (See py_matrix)
...
//    PyModule.def(diag, "diag");
//    PyModule.def(inv,  "inv");

//              Container Support Functions (See py_matrix)
...
//              Python Specialty Functions (See py_matrix)

//    .def("__str__", &matrix::PyPrint)
//    .def("__str__", &matrix::PyPicture)
    ;

//                  Friend Functions (See py_matrix)
...
//  (See above, Delete when Boost.Python handles static methods)
    ...
    def("matrix_Header",     &matrix::Header);
    def("matrix_PrintRI",    &matrix::PrintRI);
    def("matrix_PrintAll",   &matrix::PrintAll);
    def("matrix_PictDim",    &matrix::PictDim);
    def("matrix_PrintVal",   &matrix::PrintVal);
    def("matrix_PrintCols",  &matrix::PrintCols);
    def("matrix_PrintRows",  &matrix::PrintRows);
    def("matrix_PrintFlags", &matrix::PrintFlags);

//                Additonal Functions (Thanx To MSVC++)

  PyMatrixMore(PyMx);				// More matrix stuff (VC++ 7)
  PyMatrixEvenMore(PyMx);			// More matrix stuff (VC++ 7)
  }
