...
std::string super_op::PyPrint()
  {
  std::stringstream sstr;
  std::string hdr;
  if(!dim())
    {
    hdr = std::string("Empty Liouville Space Operator");
    sstr << CenterString(hdr) << std::endl;
    return sstr.str();
    }
  hdr = std::string("Liouville Space Operator");
...
void super_op::genString(std::stringstream &sstr)
  {
  if(!LSp) sstr << "NULL Super Operator\n";
  else
  sstr << "Matrix:\n" << mx
       << "Hilbert Space Basis:\n" << Hbs
       << "Liouville Space Basis:\n" << Lbs;
...
//                                  The Class
  ...
  boost::python::class_<super_op> ("super_op", init<>())
...
//    .def(init<matrix*, int, matrix*>())
    .def(init<const super_op&>())
    .def(init<const gen_op&, const gen_op&>())
...
//    pow functions
    ...
    .def("set_EBR",    &super_op::set_EBR)
    .def("set_HBR",    &super_op::set_HBR)
    .def("set_DBR",    &super_op::set_DBR)

// LOp_base

    .def("LOp_Hbase",   &super_op::LOp_Hbase)
    .def("SetHSBaseOf", &super_op::SetHSBaseOf)

    .def("HS",      &super_op::HS)
    .def("size",    &super_op::size)
    .def("dim",     &super_op::dim)
    .def("LS",      &super_op::LS)

// eigenvalues

    .def("Mx",         &super_op::Mx)
    .def("get_mx",     &super_op::get_mx)
    .def("put_mx",     &super_op::put_mx)

    .def("Bs",         &super_op::Bs)
    .def("get_basis",  &super_op::get_basis)
    .def("put_basis",  &super_op::put_basis)

    .def("LBs",        &super_op::LBs)
    .def("get_Lbasis", &super_op::get_Lbasis)
    .def("put_Lbasis", &super_op::put_Lbasis)

// access operator

    .def("put",        &super_op::put)
    .def("get",        &super_op::get)

// read write

//                          Python Specialty Functions

  .def( "__str__", &super_op::PyPrint)
  ;

//                             Global Functions

    def("left",  (super_op (*)(const gen_op&))               &left);
    def("left",  (super_op (*)(const matrix&))               &left);
    def("left",  (super_op (*)(const matrix&, const basis&)) &left);
    def("right", (super_op (*)(const gen_op&))               &right);
    def("right", (super_op (*)(const matrix&))               &right);
    def("right", (super_op (*)(const matrix&, const basis&)) &right);

    def("commutator", (super_op (*)(const gen_op&))          &commutator);
    def("commutator", (super_op (*)(const matrix&))          &commutator);

    def("d_commutator", (super_op (*)(const gen_op&, const complex&))                &d_commutator);
    def("d_commutator", (super_op (*)(const matrix&))                                &d_commutator);
    def("d_commutator", (super_op (*)(const gen_op&, const gen_op&))                 &d_commutator);
    def("d_commutator", (super_op (*)(const gen_op&, const gen_op&, const complex&)) &d_commutator);
    def("d_commutator", (super_op (*)(const matrix&, const matrix&))                 &d_commutator);

    def("U_transform", (super_op (*)(const gen_op&))               &U_transform);
    def("U_transform", (super_op (*)(const matrix&))               &U_transform);

    def("project", (super_op (*)(const gen_op&))               &project);
    def("project", (super_op (*)(const matrix&))               &project);

    def("Hsuper", &Hsuper);
//    def("Hsuper", (super_op(*)(const gen_op&)) &Hsuper);
