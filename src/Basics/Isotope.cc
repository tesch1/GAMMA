...
string Isotope::PyPrint()
  ...
  string cstr;
  if(cl > 0) cstr = string(cl, ' ');
  for(i=0; i<ns; i++)				// Loop over strings and send
    sstr << cstr << PStrings[i] << std::endl;	// to our string stream
  return sstr.str();				// Return as Python printable string
  }

string Isotope::PyPrintList()
  {
  vector<string> PStrings;		// Array of strings to print
  PStrings = PrintListStrings();	// Get our print strings
  stringstream sstr;			// Like ostream but with strings
  unsigned ns = PStrings.size();	// No. of strings to print
...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(Isotope_read,  Isotope::read,  1, 3)

void PyIsotope()
  {
//                                  The Class

  boost::python::class_<Isotope>("Isotope", init<>())

//                                Constructors

    .def(init<const Isotope&>())
...
//                              Member Functions
    ...
    .def("qn",                 &Isotope::qn)
    .def("HS",                 &Isotope::HS)
    .def("momentum",           &Isotope::momentum)
    .def("symbol",             &Isotope::symbol,  return_value_policy<copy_const_reference>())
    .def("name",               &Isotope::name,    return_value_policy<copy_const_reference>())
    .def("element",            &Isotope::element, return_value_policy<copy_const_reference>())
    .def("number",             &Isotope::number)

    .def("mass",               &Isotope::mass)
    .def("weight",             &Isotope::weight)
    .def("gamma",              &Isotope::gamma)
    .def("receptivity",        &Isotope::receptivity)
    .def("relative_frequency", &Isotope::relative_frequency)
    .def("electron",           &Isotope::electron)

    .def("seek",               &Isotope::seek)
    .def("known",              &Isotope::known)
    .def("exists",             &Isotope::exists)
    .def("size",               &Isotope::size)

    .def("nepair",             &Isotope::nepair)
    .def("eepair",             &Isotope::eepair)
    .def("enpair",             &Isotope::enpair)
    .def("nnpair",             &Isotope::nnpair)
    .def("AddIsotope",         &Isotope::AddIsotope)

    .def("read", (bool(Isotope::*) (const std::string&,  int, int))0, Isotope_read())
    .def("read", (bool(Isotope::*) (ParameterSet&,       int, int))0, Isotope_read())
...
//                          Python Specialty Functions
    ...
    .def("__str__",     &Isotope::PyPrint)
    .def("PyPrintList", &Isotope::PyPrintList)
    ;
  }
