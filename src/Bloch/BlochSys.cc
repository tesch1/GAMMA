...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(BlochSys_H,    BlochSys::H,    0, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(BlochSys_B,    BlochSys::B,    0, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(BlochSys_read, BlochSys::read, 1, 3)

void PyBlochSys()
  {
//                                  The Class

  boost::python::class_<BlochSys>("BlochSys")

//                                Constructors

    .def(init<optional<int> >())
    .def(init<const BlochSys&>())
    .def(init<double, double, double>())

//    .def(init<const std::vector<double>&,                              const std::vector<double>&, const std::vector<double>&>())
//    .def(init<const std::vector<double>&, const std::vector<Isotope>&, const std::vector<double>&, const std::vector<double>&>())
//    .def(init<const std::vector<double>&,                              const std::vector<double>&, const std::vector<double>&, const std::vector<double>&>())
//    .def(init<const std::vector<double>&, const std::vector<Isotope>&, const std::vector<double>&, const std::vector<double>&, const std::vector<double>&>())

    .def(init<const spin_system&, const RBasic&>())
    .def(init<const spin_system&,                const matrix&>())
    .def(init<const spin_system&, const RBasic&, const matrix&>())

//    .def(init<const TTable1D&, optional<const std::string&> >())

    .def("NIso",         &BlochSys::NIso)
    .def("IsoMaxLength", &BlochSys::IsoMaxLength)
    .def("NSpins",       &BlochSys::NSpins)
    ...
    .def("MaxExchange",  &BlochSys::MaxExchange)

//    .def("Norms", (std::vector<double> (BlochSys::*)() const)                    &BlochSys::Norms)
//    .def("Norms", (void                (BlochSys::*)(const std::vector<double>&)) &BlochSys::Norms)

    .def("Norm", (double (BlochSys::*)(int) const)   &BlochSys::Norm)
    .def("Norm", (void   (BlochSys::*)(double, int)) &BlochSys::Norm)

    .def("H", (matrix(BlochSys::*) (double, double, double))0, BlochSys_H())
    .def("B", (matrix(BlochSys::*) (double, double, double))0, BlochSys_B())
    .def("R", &BlochSys::R)
    .def("K", &BlochSys::K)

    .def("Meq",           &BlochSys::Meq)
    .def("Mo",            &BlochSys::Mo,  return_value_policy<copy_const_reference>())
    .def("Mx",            &BlochSys::Mx)
    .def("My",            &BlochSys::My)
    .def("Mz",            &BlochSys::Mz)

    .def("Mss", (MagVec (BlochSys::*)(const matrix&, const matrix&)                    const) &BlochSys::Mss)
    .def("Mss", (MagVec (BlochSys::*)(const matrix&, const matrix&, const col_vector&) const) &BlochSys::Mss)

    .def("DetectMu", (row_vector (BlochSys::*)(int)                     const) &BlochSys::DetectMu)
    .def("DetectMu", (row_vector (BlochSys::*)(int,                int) const) &BlochSys::DetectMu)
    .def("DetectMu", (row_vector (BlochSys::*)(const std::string&, int) const) &BlochSys::DetectMu)

    .def("DetectMx", (row_vector (BlochSys::*)() const) &BlochSys::DetectMx)
    .def("DetectMy", (row_vector (BlochSys::*)() const) &BlochSys::DetectMy)
    .def("DetectMz", (row_vector (BlochSys::*)() const) &BlochSys::DetectMz)

    .def("DetectMx", (row_vector (BlochSys::*)(int) const) &BlochSys::DetectMx)
    .def("DetectMy", (row_vector (BlochSys::*)(int) const) &BlochSys::DetectMy)
    .def("DetectMz", (row_vector (BlochSys::*)(int) const) &BlochSys::DetectMz)

    .def("DetectMx", (row_vector (BlochSys::*)(const std::string&) const) &BlochSys::DetectMx)
    .def("DetectMy", (row_vector (BlochSys::*)(const std::string&) const) &BlochSys::DetectMy)
    .def("DetectMz", (row_vector (BlochSys::*)(const std::string&) const) &BlochSys::DetectMz)

    .def("size",          &BlochSys::size)

    .def("read", (bool(BlochSys::*) (const std::string&,  int, int))0, BlochSys_read())
    .def("read", (bool(BlochSys::*) (const ParameterSet&, int, int))0, BlochSys_read())
