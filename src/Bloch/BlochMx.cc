...
std::string BlochMx::PyPrint() const
  ...
  if(!rows())
    {
    hdr = std::string("Empty Bloch Matrix");
    sstr << CenterString(hdr) << std::endl;
    return sstr.str();
    }
  hdr = std::string("Bloch Matrix - ");
  int n = NComps();
  hdr += Gdec(n) + " Component";
  if(n > 1) hdr += "s";
  sstr << CenterString(hdr) << std::endl;
...
//                                  The Class
  ...
  class_<BlochMx, bases<matrix> >("BlochMx")

//                                Constructors

    .def(init<>())
    .def(init<const BlochMx&>())
...
//                            Member Functions
    ...
    .def("NComps", &BlochMx::NComps)

    .def(str(self))

//                          Python Specialty Functions

    .def("__str__",     &BlochMx::PyPrint)
  ;
  }
