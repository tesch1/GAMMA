...
double d1halfA(int n, double beta)            { return d1half(   n, beta); }
double d1halfB(int m, int n, double beta )    { return d1half(m, n, beta); }
double dJA(int J, int m, int n, double beta ) { return dJ(J,  m, n, beta); }
matrix dJB(int J, double beta)                { return dJ(J,        beta); }

matrix DJA(const matrix& dJbeta, int J, double alpha)
  { return DJ(dJbeta, J, alpha); }

void PyWigner()
  {
  def("d0", d0);

//            We Must Deal With Function d1half Overloads Explicitly
  ...
  def("fact",        fact);
  def("dJ_int",      dJ_int);
  def("dJ_half_int", dJ_half_int);
  def("dJ",          dJA);

//            We Must Deal With Function dJ Overloads Explicitly

  def("dJ",     dJB);

//            Function DJ Has Export Problems For Unknown Reasons
