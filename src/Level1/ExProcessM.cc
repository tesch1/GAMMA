...
double ExchProcM::KexA() const    { return Kex(); }
void   ExchProcM::KexB(double k)  { Kex(k);       }

// ____________________________________________________________________________
// F                       PyGAMMA Code (Non-Member)
// ____________________________________________________________________________
...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(ExchProcM_read,  ExchProcM::read,  1, 3)

void PyExchProcM()
  {
//                                  The Class

  boost::python::class_<ExchProcM>("ExchProcM", init<>())

//                                Constructors

    .def(init<const ExchProcM&>())        

//                              Member Functions

    .def("Kex", &ExchProcM::KexA)
    .def("Kex", &ExchProcM::KexB)

    .def("NComps", &ExchProcM::NComps)
    .def("NSpins", &ExchProcM::NSpins)
    .def("Comp",   &ExchProcM::Comp)

    .def("mixes",    &ExchProcM::mixes)
    .def("involves", &ExchProcM::involves)

    .def("read", (bool(ExchProcM::*) (const std::string&,  int, int))0, ExchProcM_read())
    .def("read", (bool(ExchProcM::*) (ParameterSet&,       int, int))0, ExchProcM_read())

    .def("ExchStr", &ExchProcM::ExchStr)

//                          Python Specialty Functions

    .def("__str__", &ExchProcM::PyPrint)
    ;
  }
