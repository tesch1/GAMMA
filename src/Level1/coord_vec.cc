...
coord_vec coord_vec::rotateB(double alpha, double beta, double gamma) const
  { return rotate(alpha, beta, gamma); }
coord_vec coord_vec::rotateC(const coord& EA) const
  { return rotate(EA); }

void coord_vec::rotate_ipA(double alpha, double beta, double gamma)
  { coord_vec::rotate_ip(alpha, beta, gamma); }
void coord_vec::rotate_ipB(const coord &EA)
  { coord_vec::rotate_ip(EA); }

coord coord_vec::maximaA() const                                { return maxima(); }
void  coord_vec::maximaB(double &x, double &y, double &z) const { maxima(x,y,z);   }

double coord_vec::max_RA() const                         { return max_R();    }
void   coord_vec::max_RB(int &maxi, double &maxR) const  { max_R(maxi, maxR); }

void coord_vec::putA(const coord &pt1, int index)             { put(pt1,     index); }
void coord_vec::putB(double x, double y, double z, int index) { put(x, y, z, index); }

string coord_vec::PyprintCylindrical(double sf) const
  {
  stringstream sstr;
  for(int i=0; i<Npts; i++)
    sstr << "\n" << (sf*Pts[i]).Cart2Cyl(0);
...
string coord_vec::PyprintSpherical(double sf) const
  {
  stringstream sstr;
  for(int i=0; i<Npts; i++)
    sstr << "\n" << (sf*Pts[i]).Cart2Sph(0);
...
string coord_vec::PyprintCartesian(double sf) const
  {
  stringstream sstr;
  for(int i=0; i<Npts; i++) sstr << "\n" << Pts[i]*sf;
  return sstr.str();
  }

string coord_vec::PyPrint() const
  {
  int N  = Npts;				// Insure # points OK
  int NC = 1;					// Insure # columns OK
  int cp = 0;
  string lst("\n");				// Line start string
  string cs("   ");				// Column spacing
  int ilen = (Gdec(N-1)).length();		// Width of index string
  ...
  for(int i=0; i<N; i++)
    {
    if(!cp) sstr << lst;			// Either write line start
    else    sstr << cs;				// or space to new column
    sstr << Gdec(i, ilen) << ". ";		// Write the point index
    sstr << Pts[i];				// Write the point itself
    cp++;					// Update column count
    if(cp >= NC) cp = 0;			// Adjust if N columns output
    }
  return sstr.str();
...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_xrotate, coord_vec::xrotate, 1, 2)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_yrotate, coord_vec::yrotate, 1, 2)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_zrotate, coord_vec::zrotate, 1, 2)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_trans,   coord_vec::translate,     1, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_transip, coord_vec::translate_ip,  1, 3)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_distance,  coord_vec::distance,  2, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_distances, coord_vec::distances, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_thetas,    coord_vec::thetas,    0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_phis,      coord_vec::phis,      0, 1)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_read,    coord_vec::read,    1, 3)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_Cart2Sph, coord_vec::Cart2Sph, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_Sph2Cart, coord_vec::Sph2Cart, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_Cart2Cyl, coord_vec::Cart2Cyl, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_Cyl2Cart, coord_vec::Cyl2Cart, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_Sph2Cyl,  coord_vec::Sph2Cyl,  0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(coordvec_Cyl2Sph,  coord_vec::Cyl2Sph,  0, 1)

void PyCoordVec()
  {
//                                  The Class

  boost::python::class_<coord_vec> ("coord_vec", init<>())

//                                Constructors

    .def(init<int>())
    .def(init<const coord_vec&>())
    .def(init<const ParameterSet&, int, int>())
    .def(init<const row_vector&, const row_vector&, const row_vector&>())

//                              Member Functions

    .def("xrotate", (coord_vec(coord_vec::*) (double, int))0, coordvec_xrotate())
    .def("yrotate", (coord_vec(coord_vec::*) (double, int))0, coordvec_yrotate())
    .def("zrotate", (coord_vec(coord_vec::*) (double, int))0, coordvec_zrotate())

    .def("rotate", &coord_vec::rotateA)
    .def("rotate", &coord_vec::rotateB)
    .def("rotate", &coord_vec::rotateC)

    .def("rotate_ip", &coord_vec::rotate_ipA)
    .def("rotate_ip", &coord_vec::rotate_ipB)

//    .def("translate",    (coord_vec(coord_vec::*) (double, double, double ))0, coordvec_trans())
//    .def("translate_ip", (void(coord_vec::*)      (double, double, double ))0, coordvec_transip())

    .def("trans_x", &coord_vec::trans_x)
    .def("trans_y", &coord_vec::trans_y)
    .def("trans_z", &coord_vec::trans_z)

    .def("trans_x_ip", &coord_vec::trans_x_ip)
    .def("trans_y_ip", &coord_vec::trans_y_ip)
    .def("trans_z_ip", &coord_vec::trans_z_ip)

    .def("project", &coord_vec::project)
...
//                                  Operators
    ...
    .def("size",   &coord_vec::size)
    .def("max_x",  &coord_vec::max_x)
    .def("max_y",  &coord_vec::max_y)
    .def("max_z",  &coord_vec::max_z)

    .def("maxima", &coord_vec::maximaA)
    .def("maxima", &coord_vec::maximaB)

    .def("max_R", &coord_vec::max_RA)
    .def("max_R", &coord_vec::max_RB)

    .def("vectors",   &coord_vec::vectors)
    .def("vectors_f", &coord_vec::vectors_f)

    .def("distance",  (double(coord_vec::*) (int, int, int))0, coordvec_distance())
    .def("distances", (matrix(coord_vec::*) (int          ))0, coordvec_distances())
    .def("thetas",    (matrix(coord_vec::*) (int          ))0, coordvec_thetas())
    .def("phis",      (matrix(coord_vec::*) (int          ))0, coordvec_phis())

//                   The access operator () has not been exported

    .def("put", &coord_vec::putA)
    .def("put", &coord_vec::putB)
    .def("get", &coord_vec::get)
    .def("x",   &coord_vec::x)
    .def("y",   &coord_vec::y)
    .def("z",   &coord_vec::x)

    .def("get_block", &coord_vec::get_block)
    .def("put_block", &coord_vec::put_block)

//                The parameter set functions have not been exported
//                     The write functions have not been exported

    .def("is_zero", &coord_vec::is_zero)

    .def("read", (int(coord_vec::*) (const std::string&,  int, int))0, coordvec_read())
    .def("read", (int(coord_vec::*) (ParameterSet&,       int, int))0, coordvec_read())

//                      The function ask_read has not been exported

    .def("Cart2Sph", (coord_vec(coord_vec::*) (int))0, coordvec_Cart2Sph())
    .def("Sph2Cart", (coord_vec(coord_vec::*) (int))0, coordvec_Sph2Cart())
    .def("Cart2Cyl", (coord_vec(coord_vec::*) (int))0, coordvec_Cart2Cyl())
    .def("Cyl2Cart", (coord_vec(coord_vec::*) (int))0, coordvec_Cyl2Cart())
    .def("Sph2Cyl",  (coord_vec(coord_vec::*) (int))0, coordvec_Sph2Cyl())
    .def("Cyl2Sph",  (coord_vec(coord_vec::*) (int))0, coordvec_Cyl2Sph())

//                             Python Specialty Functions

    .def("__str__", &coord::PyPrint)
  ;
