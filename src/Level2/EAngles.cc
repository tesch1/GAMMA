...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EA_param1,  EAngles::param,     1, 2)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EA_param2,  EAngles::param,     2, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EA_PSetAdd, EAngles::param,     1, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EA_write,   EAngles::write,     1, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EA_read,    EAngles::read,      1, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EA_SetCut,  EAngles::SetCutoff, 0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EA_equal,   EAngles::equal,     1, 2)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EA_RMx,     EAngles::RMx,       0, 1)
...
//                                  The Class
  ...
  boost::python::class_<EAngles>("EAngles", init<>())

//                                Constructors

    .def(init<const EAngles&>())
    .def(init<double, optional<double, double, bool> >())
    .def(init<const coord&, optional<bool> >())

//                           Member Functions

    .def("alpha", (double (EAngles::*)() const) &EAngles::alpha)
    .def("beta",  (double (EAngles::*)() const) &EAngles::beta)
    .def("gamma", (double (EAngles::*)() const) &EAngles::gamma)
    .def("alpha", (void   (EAngles::*)(double)) &EAngles::alpha)
    .def("beta",  (void   (EAngles::*)(double)) &EAngles::beta)
    .def("gamma", (void   (EAngles::*)(double)) &EAngles::gamma)
...
//                               Operators
    ...
    .def("composite", &EAngles::composite)

    .def("param", (SinglePar(EAngles::*) (const std::string&,                     bool))0, EA_param1())
    .def("param", (SinglePar(EAngles::*) (const std::string&, const std::string&, bool))0, EA_param2())
//    .def(ParameterSet_(self))
//    .def(ParameterSet() += self)                

    .def("write", (void(EAngles::*) (const std::string&,  int, bool))0, EA_write())
    .def("read",  (bool(EAngles::*) (const std::string&,  int, int))0,  EA_read())
    .def("read",  (bool(EAngles::*) (ParameterSet&,       int, int))0,  EA_read())

    .def(str(self)) 

// SetCutoff is a static method and not properly exposed in Boost.Python.
// As such in must be exported at the module scope, not within the class.
// A special function is used to do that task (see below). When Boost.Python
// properlay handles static medthods then switch back to use the member function
// defined here.

//  .def("SetCutoff", (void(EAngles::*) (double))0, EA_SetCut())
...
//                         Container Support Functions
    ...
    .def("equal",   (bool   (EAngles::*) (const EAngles&, double))0, EA_equal())
    .def("inverse", &EAngles::inverse)
    .def("RMx",     (matrix (EAngles::*) (bool))0,                   EA_RMx())
  ;

//                  Static Methods ==> Global Functions
//  (See above, Delete when Boost.Python handles static methods)

  def("EAngles_SetCutoff", &EAngles::SetCutoff);

//                           Global Constants

  boost::python::scope().attr("EAzero") = EAzero;
  }
