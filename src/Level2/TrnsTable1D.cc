...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(ttab_center,   TTable1D::center,       0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(ttab_offset,   TTable1D::offset,       1, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(ttab_FRscale,  TTable1D::FRscale,      1, 2)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(ttab_BC,       TTable1D::BC,           0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(ttab_Iscale,   TTable1D::Iscale,       0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(ttab_Iremove,  TTable1D::Iremove,      0, 1)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(ttab_broaden,  TTable1D::ttab_broaden, 1, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(ttab_setInorm, TTable1D::setInorm,     0, 1)

void PyTrnsTable()
  {
//                                  The Class

  boost::python::class_<TTable1D> ("TTable1D", init<>())

//                                Constructors

    .def(init<const matrix&>())
    .def(init<const matrix&, int>())
//    .def(init<const TTable1D&>())

//                              Member Functions

    .def("center",  (double (TTable1D::*)(bool))                &TTable1D::center,  "ttab_center")
    .def("offset",  (void   (TTable1D::*)(double, int, int))    &TTable1D::offset,  "ttab_offset")
    .def("FRscale", (void   (TTable1D::*)(double, int))         &TTable1D::FRscale, "ttab_FRscale")
    .def("BC",      &TTable1D::BC,      ttab_BC())

    .def("Iscale",  (void   (TTable1D::*)(double, int))         &TTable1D::Iscale,  "ttab_Iscale")
    .def("Iscale",  (void   (TTable1D::*)(const complex&, int)) &TTable1D::Iscale,  "ttab_Iscale")
    .def("Iremove", (void   (TTable1D::*)(double))              &TTable1D::Iscale,  "ttab_Iremove")
    .def("broaden", (void   (TTable1D::*)(double, int, int))    &TTable1D::offset,  "ttab_broaden")

    .def("resolution", &TTable1D::resolution)

    .def("T",  (row_vector (TTable1D::*)(int, double)              const) &TTable1D::T)
    .def("T",  (void       (TTable1D::*)(row_vector& data, double) const) &TTable1D::T)
//  .def("Ts",       &TTable1D::Ts)
//  .def("TCutoffs", &TTable1D::TCutoffs)

    .def("F",  (row_vector (TTable1D::*)(int, double, double)         const) &TTable1D::F)
    .def("F",  (void       (TTable1D::*)(row_vector&, double, double) const) &TTable1D::F)
//  .def("Fs", &TTable1D::Fs)

    .def("FD", (row_vector (TTable1D::*)(int, double, double)         const) &TTable1D::FD)
    .def("FD", (void       (TTable1D::*)(row_vector&, double, double) const) &TTable1D::FD)

  .def("pcorrect", (complex (TTable1D::*)(double &, double, int)) &TTable1D::pcorrect)
  .def("pcorrect", (void    (TTable1D::*)(double, complex&))    &TTable1D::pcorrect)
    ...
    .def("LineWidths",  &TTable1D::LineWidths)
    .def("Intensities", &TTable1D::Intensities)
    .def("Phases",      &TTable1D::Phases)
    .def("size",        &TTable1D::size)
    .def("FRmax",       &TTable1D::FRmax)
    .def("FRmin",       &TTable1D::FRmin)
    .def("Tdmin",       &TTable1D::Tdmin)
    .def("LWmax",       &TTable1D::LWmax)
    .def("LWmin",       &TTable1D::LWmin)
    .def("Imax",        &TTable1D::Imax)
    .def("Noisemax",    &TTable1D::Noisemax)
    .def("mx",          &TTable1D::mx)
//  .def("Sort",        &TTable1D::Sort)

// Parameter Set Functions Not All Exported

    .def("setType",         &TTable1D::setType)
    .def("setSort",        &TTable1D::setSort)
    .def("setConv",        &TTable1D::setConv)
    .def("setIcut",        &TTable1D::setIcut)
    .def("setInorm",       (void  (TTable1D::*)(double)) &TTable1D::setInorm, "ttab_setInorm")
    .def("setSN",          &TTable1D::setSN)
    .def("setHprint",      &TTable1D::setHprint)
    .def("setRprint",      &TTable1D::setRprint)
    .def("setLWprint",     &TTable1D::setLWprint)
    .def("setT2print",     &TTable1D::setT2print)
    .def("setPHprint",     &TTable1D::setPHprint)
    .def("setFreqRev",     &TTable1D::setFreqRev)

    .def("getType",        &TTable1D::getType)
    .def("getSort",        &TTable1D::getSort)
    .def("getConv",        &TTable1D::getConv)
    .def("getIcut",        &TTable1D::getIcut)
    .def("getInorm",       &TTable1D::getInorm)
    .def("getSN",          &TTable1D::getSN)
    .def("getHprint",      &TTable1D::getHprint)
    .def("getRprint",      &TTable1D::getRprint)
    .def("getLWprint",     &TTable1D::getLWprint)
    .def("getT2print",     &TTable1D::getT2print)
    .def("getPHprint",     &TTable1D::getPHprint)
    .def("getFreqRev",     &TTable1D::getFreqRev)

// read write print functions not all exported yet

//    .def((void  (TTable1D::*)(const std::string&)) &TTable1D::read);

//                           Python Specialty Functions

    .def("__str__", &TTable1D::PyPrint)
  ;
