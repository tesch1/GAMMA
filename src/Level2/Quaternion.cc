...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(quatern_composite, quatern::composite, 1, 2)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(quatern_param,     quatern::param,     0, 2)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(quatern_write,     quatern::write,     1, 4)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(quatern_read,      quatern::read,      1, 3)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(quatern_ValidRMx,  quatern::ValidRMx,  1, 2)

void PyQuaternion()
  {
//                                  The Class

  boost::python::class_<quatern>("quatern", init<>())

//                                Constructors

    .def(init<const coord&,        optional<bool> >())
    .def(init<const EAngles&,      optional<bool> >())
    .def(init<const quatern&,      optional<bool> >())
    .def(init<const ParameterSet&, optional<int, int> >())
    .def(init<double, double, double, double, optional<bool> >())

// The Assignments have not been exported
...
//                           Member Functions
    ...
    .def("alpha", &quatern::alpha)
    .def("beta",  &quatern::beta)
    .def("gamma", &quatern::gamma)
...
//                               Operators
    ...
    .def("composite", (quatern (quatern::*) (const quatern&, bool))0, quatern_composite())
    .def("RotMx",     &quatern::RotMx)
    .def("RMx",       &quatern::RMx)

    .def("norm",      &quatern::norm)
    .def("inverse",   &quatern::inverse)
...
//    .def("param", (quatern (quatern::*) (const std::string&, const std::string&))0, quatern_param())

//    .def(ParameterSet_(self))
//    .def(ParameterSet() += self) 

    .def("write", (bool(quatern::*) (const std::string&,  int, int, int))0, quatern_write())
    .def("write", (bool(quatern::*) (std::ofstream&,      int, int, int))0, quatern_write())
    .def("read",  (bool(quatern::*) (const std::string&,  int, int))0,      quatern_read())
    .def("read",  (bool(quatern::*) (ParameterSet&,       int, int))0,      quatern_read())

// ASinPos, ACosPos, ATanPos, ValidRMx are all static methods and not properly
// exposed in Boost.Python. As such  they must be exported at the module scope,
// not within the class. Special functions are used to do that task (see below).
// When Boost.Python properlay handles static medthods then switch back to use
// the member functions defined here.

//  .def("ASinPos",        &quatern::ASinPos)
//  .def("ACosPos",        &quatern::ACosPos)
//  .def("ATanPos",        &quatern::ATanPos)
    .def("ShowConversion", &quatern::ShowConversion)
//  .def("ValidRMx", (bool (quatern::*) (const matrix&, bool))0, quatern_ValidRMx())
  ;

//                                Global Functions

  def("composite", (quatern (*)(const EAngles&, const EAngles&)) &composite);
  def("composite", (quatern (*)(const coord&,   const coord&))   &composite);
  def("composite", (quatern (*)(const quatern&, const quatern&)) &composite);
  def("composite", (quatern (*)(const matrix&,  const quatern&)) &composite);

//                  Static Methods ==> Global Functions
//  (See above, Delete when Boost.Python handles static methods)

  def("quatern_ASinPos",  &quatern::ASinPos);
  def("quatern_ACosPos",  &quatern::ACosPos);
  def("quatern_ATanPos",  &quatern::ATanPos);
  def("quatern_ValidRMx", (bool (quatern::*) (const matrix&, bool))0, quatern_ValidRMx());
  }
