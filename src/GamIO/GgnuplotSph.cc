...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(GPSPH_DataAxisFile, GPSphere::DataAxisFile, 1, 2)
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(GPSPH_DataFile,     GPSphere::DataFile,     1, 2)

BOOST_PYTHON_FUNCTION_OVERLOADS(GPSPH_GP_sphere,     GP_sphere,     2, 3)
BOOST_PYTHON_FUNCTION_OVERLOADS(GPSPH_GP_sphereplot, GP_sphereplot, 2, 3)

void PyGgnuplotSph()
  {
//                                  The Class

  boost::python::class_<GPSphere>("GPSphere", init<>())

//                                Constructors

    .def(init<const GPSphere&>())

//                              Member Functions

    .def("SetDegrees",       &GPSphere::SetDegrees)
    .def("SetSphereAxes",    &GPSphere::SetSphereAxes)
    .def("SetBaseSphere",    &GPSphere::SetBaseSphere)
    .def("SetNormalization", &GPSphere::SetNormalization)

    .def("LoadFile", &GPSphere::LoadFile)

    .def("DataAxisFile", &GPSphere::DataAxisFile, GPSPH_DataAxisFile())
    .def("DataFile",     &GPSphere::DataFile,     GPSPH_DataFile())
    .def("DataFiles",    &GPSphere::DataFiles)

    .def("Plot", (void (GPSphere::*)())                              &GPSphere::Plot)
    .def("Plot", (void (GPSphere::*)(const coord_vec&))              &GPSphere::Plot)
    .def("Plot", (void (GPSphere::*)(const std::vector<coord_vec>&)) &GPSphere::Plot)
...
//                              Global Functions
  ...
  def("GP_sphere",     (void(*)(const std::string&, const coord_vec&,   bool))0, GPSPH_GP_sphere());
  def("GP_sphereplot", (void(*)(const std::string&, const std::string&, bool))0, GPSPH_GP_sphereplot());
  def("GP_sphereplot", (void(*)(const std::string&, int, std::string*))               &GP_sphereplot);
  def("GnuplotSphere", GnuplotSphere);
