...
/* These functions are only for the PyGAMMA compilation. There are several
   reasons we need these functions. First, the Python interpreter does not
   handle standard output & ostreams well. Hence we must adjust any functions
   that output such a stream. Second, Boost.Python does not readily export
   functions that have default argument values. Consequently, we need to
   make functions for every possible combination of input as needed.         */


#ifdef PYGAMMA					// Begin PyGAMMA code block

void GP_1DGP1(const std::string& filename, const row_vector &vx)
  { GP_1D(filename, vx); }

void GP_1DGP2(const std::string& filename, const row_vector &vx, int ri)
  { GP_1D(filename, vx, ri); }
 
 
void Gnuplot1DGP1(const std::string& name, const row_vector& vx)
  { Gnuplot1D(name, vx); }

void Gnuplot1DGP2(const std::string& name, const row_vector& vx, int ri)
  { Gnuplot1D(name, vx, ri); }

void Gnuplot1DGP3(const std::string& name, const row_vector& vx, int ri, double xmin, double xmax)
  { Gnuplot1D(name, vx, ri, xmin, xmax); }

//void Gnuplot1DGP3(const std::string& name,  const std::vector<row_vector>& vxs)
//  { Gnuplot1D(name, vxs); }

//void Gnuplot1DGP4(const std::vector<std::string>& names, const std::vector<row_vector>& vxs)
//  { Gnuplot1D(names, vxs); }

void Vstring_push_back(std::vector<std::string>& vs, const std::string& str)
  { vs.push_back(str); }

void Vrowvec_push_back(std::vector<row_vector>&  vv, const row_vector& rvec)
  { vv.push_back(rvec); }
...
BOOST_PYTHON_FUNCTION_OVERLOADS(Ggnuplot1D_GP_1D,       GP_1D,     2, 6)
BOOST_PYTHON_FUNCTION_OVERLOADS(Ggnuplot1D_GP_1Dma,     GP_1Dm,    3, 7)
BOOST_PYTHON_FUNCTION_OVERLOADS(Ggnuplot1D_GP_1Dmb,     GP_1Dm,    2, 6)
BOOST_PYTHON_FUNCTION_OVERLOADS(Ggnuplot1D_GP_1Dplota,  GP_1Dplot, 2, 3)
BOOST_PYTHON_FUNCTION_OVERLOADS(Ggnuplot1D_GP_1Dplotb,  GP_1Dplot, 4, 5)
BOOST_PYTHON_FUNCTION_OVERLOADS(Ggnuplot1D_GP_1Dplotc,  GP_1Dplot, 3, 4)
BOOST_PYTHON_FUNCTION_OVERLOADS(Ggnuplot1D_GP_1Dplotd,  GP_1Dplot, 2, 3)
BOOST_PYTHON_FUNCTION_OVERLOADS(Ggnuplot1D_Ggnuplot1Da, Gnuplot1D, 2, 7)
BOOST_PYTHON_FUNCTION_OVERLOADS(Ggnuplot1D_Ggnuplot1Db, Gnuplot1D, 2, 8)

void PyGgnuplot1D()
  {
  def("GP1DXAxis",   (std::vector<double> (*)(const GPdat&, int                         ))      &GP1DXAxis);
  def("GP1DPtFlags", (std::vector<int>    (*)(const GPdat&, int, const row_vector&, bool))      &GP1DPtFlags);
  def("GPXYData",    (void                (*)(std::ofstream&, const row_vector&, const GPdat&)) &GPXYData);

  def("GPXYLoad", GPXYLoad);


  def("GP_1D", (void (*)(std::ofstream&,     const row_vector&, const GPdat&))                 &GP_1D);
  def("GP_1D", (void (*)(const std::string&, const row_vector&, const GPdat&))                 &GP_1D);
  def("GP_1D", (void (*)(std::ofstream&,     const row_vector&, int, double, double, double))0, Ggnuplot1D_GP_1D());
  def("GP_1D", (void (*)(const std::string&, const row_vector&, int, double, double, double))0, Ggnuplot1D_GP_1D());

  def("GP_1Dm", (void (*)(const std::string&,              row_vector*,                   int, int, double, double, double))0, Ggnuplot1D_GP_1Dma());
//  def("GP_1Dm", (void (*)(const std::string&,              const std::vector<row_vector>,      int, double, double, double))0, Ggnuplot1D_GP_1Dmb());
//  def("GP_1Dm", (void (*)(const std::vector<std::string&>, const std::vector<row_vector>,      int, double, double, double))0, Ggnuplot1D_GP_1Dmb());

  def("GP_1Dplot", (void (*)(const GPdat&)) &GP_1Dplot);
  def("GP_1Dplot", (void (*)(const std::string&, const std::string&, int))0,                                         Ggnuplot1D_GP_1Dplota());
  def("GP_1Dplot", (void (*)(const std::string&, const std::string&, const std::string&, const std::string&, int))0, Ggnuplot1D_GP_1Dplotb());
  def("GP_1Dplot", (void (*)(const std::string&, const std::string&, const std::string&,                     int))0, Ggnuplot1D_GP_1Dplotd());
//  def("GP_1Dplot", (void (*)(const std::string&, int, std::string*,                                          int))0, Ggnuplot1D_GP_1Dplotc());
//  def("GP_1Dplot", (void (*)(const std::string&, const std::vector<std::string>&,                            int))0, Ggnuplot1D_GP_1Dplotd());

  def("Gnuplot1D", (void (*)(const std::string&,              const row_vector&,              int, double, double,      int, double))0, Ggnuplot1D_Ggnuplot1Da());
//  def("Gnuplot1D", (void (*)(const std::string&,              const std::vector<row_vector>&, int, double, double, bool int, double))0, Ggnuplot1D_Ggnuplot1Db());
//  def("Gnuplot1D", (void (*)c(onst std::vector<std::string>&, const std::vector<row_vector>&, int, double, double, bool int, double))0, Ggnuplot1D_Ggnuplot1Db());

  def("GP_1Dblurb", GP_1Dblurb);
  }
