...
//                                  The Class
  ...
  boost::python::class_<floq_op>("floq_op", init<>())

//                                Constructors

    .def(init<const floq_op&>())
    .def(init<int, int, double>())
    .def(init<int, int, double, const matrix&>())
    .def(init<int, int, double, const matrix&, const basis&>())

//                           Member Functions

    .def("hsdim",  &floq_op::hsdim)
    .def("phodim", &floq_op::phodim)
    .def("omega",  &floq_op::omega)
    .def("dim",    &floq_op::dim)
    .def("size",   &floq_op::size)
...
//    .def(     -  self
    ...
    .def("get_block",   &floq_op::get_block)
    .def("put_block",   &floq_op::put_block)
    .def("put_sdiag",   &floq_op::put_sdiag)

    .def("put", (void    (floq_op::*)(const complex&, int, int))       &floq_op::put)
    .def("get", (complex (floq_op::*)(                int, int) const) &floq_op::get)

    .def("put", (void    (floq_op::*)(const complex&, int, int, int, int))       &floq_op::put)
    .def("get", (complex (floq_op::*)(                int, int, int, int) const) &floq_op::get)

    .def("set_DBR",   &floq_op::set_DBR)
    .def("set_EBR",   &floq_op::set_EBR)

    .def(str(self)) 

    .def("add_omega",   &floq_op::add_omega)
    .def("sub_omega",   &floq_op::sub_omega)
  ;

//				Global Functions

  def("pho_trace", (gen_op  (*)(const floq_op&))   &pho_trace);
  def("exp",       (floq_op (*)(const floq_op&))   &exp);
  def("prop",      (floq_op (*)(floq_op&, double)) &prop);
  def("fprop",     (floq_op (*)(floq_op&, double)) &fprop);
  }
