...
macro (get_mex_option option_name)
  ...
  foreach (name ${ARG_NAMES})
    # first line containing ${name}
    string(REGEX MATCH "${name}[^\r\n]*" option_line ${mexv_output})
    if ( option_line )
      # replace entire string with capturing group (after = )
      string(REGEX REPLACE "[^=:]+[=:](.*)" "\\1" value ${option_line})
      string(STRIP ${value} svalue)
      break()
    endif()
  endforeach()
  if ( svalue )
    string(REGEX REPLACE "^\"(.*)\"$" "\\1" svalue ${svalue})
    # starting in matlab 2014b, at least on mac, i needed to remove quotes from around the result
  ...
  #message( STATUS "MEX_OPTION: set to ${option_name} = ${MEX_${option_name}}" )
endmacro()
...
function(mex_setup)
  ...
  set (MEX_ARGS ${ARGV})
  execute_process(COMMAND ${mex} ${MEX_ARGS} -v ${CMAKE_CURRENT_BINARY_DIR}/dummy.c 
  ...
  set (MEXLIB_LDFLAGS ${MEX_LDFLAGS} ${MEX_LD_ARGUMENTS} ${MEX_CLIBS} ${MEX_LINKLIBS})
  # removed "-ldl") # note: the -ldl here might be overkill? so far only needed it for 
  # drake_debug_mex. (but it has to come later in the compiler arguments, too, in order to work.
  if (NOT WIN32) # AND (CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX))
    set(MEXLIB_LDFLAGS ${MEXLIB_LDFLAGS} "-ldl")
  endif()
  string(REPLACE "-bundle" "" MEXLIB_LDFLAGS "${MEXLIB_LDFLAGS}")
  # zap the exports definition file
  string(REGEX REPLACE "[ ;][^ ;]*mexFunction.map\"*" "" MEXLIB_LDFLAGS "${MEXLIB_LDFLAGS}")
  ...
  # note: on ubuntu, gcc did not like the MEX_CLIBS coming along with LINK_FLAGS 
  # (it only works if they appear after the input files). this is a nasty trick that I found online
  if (CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX)
    ...
    #message (STATUS "liblast LIBRARIES: ${MEXLIB_LDFLAGS}")
  endif()
  ...
  if (NOT compilers_match)
    message(WARNING "Your cmake C compiler is: \"${CMAKE_C_COMPILER}\" but your mex "
      "options use: \"${MEX_CC}\" (the compiler version strings are printed above). You should "
      "use the same compilers. You can either:\n a) reconfigure the mex compiler by running "
      "'mex -setup' in MATLAB, or\n b) Set the default compiler for cmake by setting the CC "
      "environment variable in your terminal.\n")
  endif()

  compare_compilers(compilers_match "${CMAKE_CXX_COMPILER}" "${MEX_CXX}")
  if (NOT compilers_match)
    message (WARNING "Your cmake CXX compiler is: \"${CMAKE_CXX_COMPILER}\" but your mex "
      "options end up pointing to: \"${MEX_CXX}\". You should use the same compilers. You can "
      "either:\n a) Configure the mex compiler by running 'mex -setup' in MATLAB, or \n b) Set "
      "the default compiler for cmake by setting the CC environment variable in your terminal.")
  endif()

  # NOTE: would like to check LD also, but it appears to be difficult with cmake 
  # (there is not explicit linker executable variable, only the make rule), and even
  # my mex code assumes that LD==LDCXX for simplicity.

endfunction() # mex_setup()

function (add_mex)
  # useage: add_mex(target source1 source2 [SHARED,EXECUTABLE])
  # note: builds the mex file inplace (not into some build directory)
  # if SHARED is passed in, then it doesn't expect a mexFunction symbol to be 
  # defined, and compiles it to e.g., libtarget.so, for eventual linking against 
  # another mex file if EXECUTABLE is passed in, then it adds an executable target, 
  # which is linked against the appropriate matlab libraries.
  ...
  if (isexe GREATER -1)
    ...
    if (CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX)
      ...
      set_target_properties(${target} PROPERTIES
        LINK_FLAGS          "${LINK_FLAGS} ${MEXLIB_LDFLAGS} ${MEX_LDOPTIMFLAGS}"
